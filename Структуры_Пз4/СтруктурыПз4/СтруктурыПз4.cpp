...
void elemdel(elem* t, char* key) {
	int k = h(key);
	if (strcmp(t[k % N].name, "") == 0 && !t[k].DELETE);
	else {
		int counter = 0;
		while (strcmp(t[k % N].name, key) != 0 && (strcmp(t[k % N].name, "") != 0 || t[k].DELETE == 1) && counter != N) {
			k = (k + 1) % N;
		}
		if (!strcmp(t[k % N].name, key)) {
			strcpy_s(t[k].name, "");
			t[k].DELETE = 1;
		}
	}
...
void vyvod(elem* t) {
	...
	for (int i = 0; i < N; i++) {
		if(strcmp(t[i].name, ""))
			fprintf(myfile, "%i %s %s %i %i %s\n",i, t[i].name, t[i].type, t[i].mem, t[i].k, t[i].type_k);
	}
}
